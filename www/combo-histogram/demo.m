...
-(NSArray*)histogram:(NSArray*)data step:(int)step {
    NSMutableDictionary *histo = [[NSMutableDictionary alloc] init];
    
    for(id value in data){
        int x = (int)floor([value[0] floatValue] / step) * step;
        if(![histo objectForKey:@(x)]) {
            [histo setObject:@0 forKey:@(x)];
        }
        int current = [[histo objectForKey:@(x)] intValue];
        [histo setObject:@(++current) forKey:@(x)];
    }
    
    NSMutableArray *arr = [[NSMutableArray alloc] init];
    
    for(id x in histo) {
        [arr addObject:@[ @([x floatValue]), @([[histo objectForKey:x] integerValue])]];
    }
    
    [arr sortedArrayUsingComparator:^NSComparisonResult(id a, id b) {
        return [a[0] floatValue] - [b[0] floatValue];
    }];
...
- (void)viewDidLoad {
    ...
    NSArray *data = [NSArray arrayWithObjects:@[@161.2, @51.6], @[@167.5, @59.0], @[@159.5, @49.2], @[@157.0, @63.0], @[@155.8, @53.6],
                     ...
                     @[@176.5, @71.8], @[@164.4, @55.5], @[@160.7, @48.6], @[@174.0, @66.4], @[@163.8, @67.3], nil];
    ...
    column.data = [self histogram:data step:10];
    ...
    options.series = [NSMutableArray arrayWithObjects:column, scatter, nil];
